...
export default function ChatWindowMobile({ onClose, onSendMessage, userName }) {
        ...
        <div className="chatWindow flex flex-col right-0 p-2 w-full h-[100vh] bg-white/60 dark:bg-darkBackground bg-opacity-80 shadow-lg rounded-2xl">
            <div className="bg-gray-100 dark:bg-darkSurface rounded-xl overflow-hidden h-[100%] relative flex flex-col">
                <div className="flex justify-between items-center p-4 shadow-sm">
                    ...
                    <button onClick={onClose} className="text-gray-500 bg-gray-200 p-1 rounded-full dark:text-white dark:bg-darkSurface">
                        <X size={25} />
                    </button>
                </div>
                    ...
                    <div className="p-6 space-y-4 flex flex-col justify-around h-[80%]">
                        ...
                        <div className="space-y-2 flex flex-col justify-around h-[45%]">
                            {['Hur skapar jag en faktura?', 'Vilka utgifter är avdragsgilla?', 'Visa min senaste månatliga rapport'].map((topic, index) => (
                                ...
                                >
                                    <div className="w-8 h-8 text-gray-100 bg-gray-800 rounded-xl flex items-center justify-center dark:bg-darkSurface ">
                                        {index === 0 ? (
                                    ...
                                    <span className="text-sm font-medium text-gray-800 dark:text-darkSurface">{topic}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="absolute bottom-0 left-0 right-0 bg-transparent p-1">
                    <div className="flex flex-row justify-between items-center space-x-2">
                        <div className="flex-grow p-2 h-auto bg-white rounded-3xl flex items-center">
                            <button
                                type="button"
                                className="p-2 bg-gray-800 text-white rounded-full hover:bg-gray-500 transition-colors duration-200 dark:bg-darkSurface"
                            >
                                <Paperclip size={20} />
                            </button>
                            <textarea
