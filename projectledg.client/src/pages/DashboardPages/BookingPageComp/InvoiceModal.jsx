...
export default function InvoiceModal({ basAccounts, invoice, isModalOpen, setIsModalOpen, handleSaveChanges,
    ...
    return (
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen} className="focus:outline-none">
            <DialogContent className="w-auto flex flex-col  min-w-[600px]  min-h-[80vh]  dark:bg-darkBackground dark:border-darkBorder focus:outline-none">
                        ...
                        <div className="FakturaInfo space-y-2 mb-3">
                            ...
                            <div className="Fakturanummer grid grid-cols-4 ">
                                ...
                                <div className="flex flex-col col-span-3 justify-between space-y-2">
                                    <p className="font-bold text-lg text-gray-600 dark:text-white mb-1">{invoice.VendorAddressRecipient}</p>
                                    <p className="text-gray-600 dark:text-white ">Belopp</p>
                                    <p className="text-gray-500 dark:text-white/90 font-light">Var av moms</p>
                                    <p className="text-gray-600 dark:text-white">Fakturadatum</p>
                                    <p className="text-gray-600 dark:text-white">Förfallodatum</p>
                                </div>
                                <div className="flex flex-col items-end space-y-2">
                                    <p className="font-bold text-lg text-gray-600 dark:text-white mb-1">{invoice.InvoiceId}</p>
                                    <p className="font-semibold text-green-600">{invoice.InvoiceTotal}kr</p>
                                    <p className="font-light text-gray-500 dark:text-white/90">{invoice.TotalTax}kr</p>
                                    <p className="text-gray-600 dark:text-white ">{invoice.InvoiceDate}</p>
                                    <p className="text-gray-600 dark:text-white ">{invoice.DueDate}</p>
                                </div>
                            </div>

                            {/* Is paid button */}
                            <div className="flex items-center justify-between">
                                <Label htmlFor="paid-status" className="font-normal text-base text-gray-600 dark:text-white">Betald</Label>
                            ...
                            <div className="p-0 h-full flex flex-col justify-between space-y-10 ">
                                <div>
                                    ...
                                    <ScrollArea ref={scrollAreaRef} className="max-h-[40vh] w-full overflow-y-auto  ">
                                        <Table className="Scroll table overflow-y-auto overflow-x-hidden w-full border dark:border-darkBorder">
                                            <TableHeader className="bg-gray-100 dark:bg-darkBorder dark:border-darkBorder">
                                                <TableRow >
                                                    <TableHead className="text-center dark:text-white/90">Konto</TableHead>
                                                    <TableHead className="text-left dark:text-white/90">Beskrivning</TableHead>
                                                    <TableHead className="text-center dark:text-white/90">Debet</TableHead>
                                                    <TableHead className="text-center dark:text-white/90">Kredit</TableHead>
                                                </TableRow>
                                            </TableHeader>
                                            <TableBody className="">
                                                {basAccounts.accounts.map((account, index) => (
                                                    <TableRow key={index} className="text-center hover:bg-gray-50 dark:hover:bg-darkSurface dark:border-darkBorder">
